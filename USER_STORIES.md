...
- **C6.** As an operator, the monitor persists my full reference band set in `settings.yml` (`monitor_bands`), so that after narrowing the lock to a single band (valid for rain or to cut packet loss) I can always widen back to the full set: the reference comes from `--bands` (which also saves it), else the saved `monitor_bands`, else a first-run bootstrap from the current lock, and it refuses to start on a single-band lock when nothing is saved. *(DONE)*
...
- **J1.** As an operator, I configure a list of test-file URLs (fast mirrors) and each probe measures achievable download bandwidth by fetching a configurable-size chunk from a randomly chosen start offset (HTTP range request) of one URL — choosing the URL at random and never reusing the immediately previous one when more than one is configured — capped by a maximum byte count and a maximum duration, so that I get a representative capacity number without hammering a single mirror or byte range, despite the firmware exposing no throughput field. *(GAP)*
